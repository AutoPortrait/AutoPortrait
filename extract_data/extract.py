...
def extract_encode_text(file_path):
...
def process_directory(root_dir):
    """遍历根目录，处理每个文件夹中的 `5_encode.txt` 文件"""
    inside_all_text = []
    outside_all_text = []
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file == "5_encode.txt":
                file_path = os.path.join(root, file)
                print(f"正在处理文件: {file_path}")
                
                # 提取 "体制内" 和 "体制外" 的文本
                inside_text, outside_text = extract_encode_text(file_path)
    ...
    # 保存处理结果到文件
    save_to_file("体制内encode.txt", "\n\n".join(inside_all_text))
    save_to_file("体制外encode.txt", "\n\n".join(outside_all_text))
    print("处理完成，结果已保存至 '体制内.txt' 和 '体制外.txt'")
